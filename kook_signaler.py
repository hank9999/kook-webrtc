...
# https://github.com/jimdragongod/mediasoup-client-pysdk/blob/main/smcdk/api/mediasoup_signaler.py
...
class KookWebRtcSignalerInterface(metaclass=ABCMeta):

    @abstractmethod
    async def connectToRoom(self, loop: asyncio.AbstractEventLoop, server_url: str, ssl_verification: bool = True):
        pass
    ...
    @abstractmethod
    async def getResponse(self, requestId: str):
        pass


class ProtooSignaler(KookWebRtcSignalerInterface):
    ...
    def generateRandomNumber() -> str:
        return str(round(random.random() * 10000000))

    async def getRouterRtpCapabilities(self) -> str:
    ...
    async def _send_request(self, requestParameters: dict) -> str:
    ...
    async def getResponse(self, requestId: str):
        ...
        try:
            ...
            return Response(requestId=message['id'], method='', data=message['data'])
...
class SignalerMessage:
    def __init__(self, requestId: str, method: str, data: dict):
